...
class CACNet_3(nn.Module):
    def __init__(
        ...
        self.similarity_weight1 = nn.Linear(256, 384)
        self.similarity_weight2 = nn.Linear(256, 384)
    ...
    def forward(self, query, sample):
        ...
        x, y = k.split([sz ** 2, 3], dim=-1)
        x = x.view(x.shape[0], x.shape[1], sz, sz)
        y = y.permute(2, 0, 1)
        #'''
        x1 = self.similarity_weight1(x.permute(0, 2, 3, 1))
        y1 = self.similarity_weight2(y)
        
        out = torch.bmm(x1.reshape((x1.shape[0], 24 ** 2, 384)), y1.permute(1, 2, 0))
