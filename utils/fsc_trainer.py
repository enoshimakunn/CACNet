...
from datasets.mosaic import FSCData_enhancement
from losses.losses import DownMSELoss
from models.ae import AutoEncoder
from models.convtrans import VGG16Trans
from models.convtrans2 import VGG16Trans2
from models.model import CACNet
from models.model_3shot import CACNet_3
from models.model_cnn import CACNet2
from models.model_vitpre import CACNet_vitpre
...
class FSCTrainer(Trainer):
    def setup(self):
        ...
        train_datasets1 = FSCData(args.data_dir,
                                 args.crop_size,
                                 args.downsample_ratio,
                                 method='train')
        train_datasets2 = FSCData_enhancement(args.data_dir,
                                 args.crop_size,
                                 args.downsample_ratio,
                                 method='train')
        #train_datasets = ConcatDataset([train_datasets1, train_datasets2])
        train_datasets = train_datasets1
        ...
        self.model = CACNet()
        #self.model = CACNet_3()
        #self.model = VGG16Trans2(dcsize=args.dcsize)
        #self.model = CACNet2()
        #self.model = CACNet_vitpre()
    ...
    def train_epoch(self):
        ...
        for inputs, targets, ex_list in tqdm(self.dataloaders['train']):
            #print(inputs.shape)
            #print(len(ex_list))
            #print(choice(ex_list[0]).shape)
            sample = torch.cat([choice(ex_list[idx]).unsqueeze(0) for idx in range(len(ex_list))])
            '''
            sample = []
            for ex in ex_list:
                if len(ex) > 3:
                    ex = ex[:3]
                sample += ex
            sample = torch.cat(sample).view(len(ex_list) * 3, 3, 16, 16)
            '''
            #print(sample.shape)
            inputs = inputs.to(self.device)
            sample = sample.to(self.device)
            targets = targets.to(self.device) * self.args.log_param
            #targets = F.interpolate(targets, scale_factor=400 / 384)

            with torch.set_grad_enabled(True):
                et_dmaps = self.model(inputs, sample)
                #et_dmaps = self.model(inputs)
                #et_dmaps = self.model.reconstruct(inputs)
                ...
                N = inputs.size(0)
                '''
                pre_count = torch.sum(et_dmaps.view(N, -1), dim=1).cuda()
                gd_count = torch.sum(targets.view(N, -1), dim=1).cuda()
                res = pre_count - gd_count
                l = nn.MSELoss()(res, torch.tensor(0.0, dtype=torch.float32, device='cuda:0'))
                l.backward()
                '''
                self.optimizer.step()
    ...
    def val_epoch(self):
        ...
        for inputs, count, ex_list, name in tqdm(self.dataloaders['val']):
            ...
            #sample = torch.cat([choice(ex_list[idx]).unsqueeze(0) for idx in range(len(ex_list))])
            sample = choice(ex_list)
            if len(ex_list) > 3:
                ex_list = ex_list[:3]
            #sample = torch.cat(ex_list).view(len(ex_list), 3, 16, 16)
            inputs = inputs.to(self.device)
            sample = sample.to(self.device)
            ...
            if h > max_size or w > max_size:
                ...
                with torch.set_grad_enabled(False):
                    ...
                    for input_ in input_list:
                        output = self.model(input_, sample)
                        pre_count += torch.sum(output) 
            else:
                with torch.set_grad_enabled(False):
                    output = self.model(inputs, sample)
                    pre_count = torch.sum(output)
